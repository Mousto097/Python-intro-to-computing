from tic_tac_toe_helper_methods import *
...
def play(tab, player):
    ...
    while not valid:
        ...
        while not((0 <= place[0] < len(tab)) and (0 <= place[1] < len(tab))):
            ...
            print(", Please provide the row and the columm from 0 to",
                  (len(tab)-1), ": ")
...
# The only array used in the program.
table = [['-', '-', '-'], ['-', '-', '-'], ['-', '-', '-']]

response = input("Start a game (O or N): ")
while response == 'o' or response == 'O':
    erasTable(table)  # prepares the game table
